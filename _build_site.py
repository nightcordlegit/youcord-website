...
/* ---------- MODAL ---------- */
.modal-overlay{position:fixed;inset:0;z-index:999;background:rgba(0,0,0,.7);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;opacity:0;pointer-events:none;transition:.25s}
.modal-overlay.show{opacity:1;pointer-events:auto}
.modal-box{background:var(--bg-2);border:1px solid var(--line);border-radius:20px;padding:36px 32px;max-width:420px;width:90%;text-align:center;transform:scale(.92);transition:.25s}
.modal-overlay.show .modal-box{transform:scale(1)}
.modal-box .icon{font-size:52px;margin-bottom:12px}
.modal-box h2{font-size:22px;font-weight:900;margin-bottom:8px}
.modal-box p{color:var(--muted);font-size:15px;margin-bottom:22px;line-height:1.5}
.modal-btns{display:flex;flex-direction:column;gap:10px}
.modal-btns .btn{padding:14px;font-size:15px;justify-content:center}
.modal-skip{background:none;border:0;color:var(--muted);font-size:13px;cursor:pointer;margin-top:12px;transition:.15s}
.modal-skip:hover{color:var(--txt)}
...
<header class="nav">
  <div class="wrap nav-in">
    ...
    <div class="nav-cta">
      <a class="btn btn-gold download-btn" href="__DOWNLOAD__">⬇ <span class="lbl">Télécharger</span></a>
    </div>
  </div>
</header>
...
<div class="wrap">
    <div class="stats">
      ...
      <div class="stat"><div class="num grad-text" id="total-count" data-count="0">0</div><div class="lbl">Total téléchargements GitHub</div></div>
      <div class="stat"><div class="num grad-text" id="win-count" data-count="0">0</div><div class="lbl">Windows</div></div>
      <div class="stat"><div class="num grad-text" id="mac-count" data-count="0">0</div><div class="lbl">macOS</div></div>
    </div>
</div>
  ...
  <div class="wrap">
    ...
    <div class="install-grid">
      <div class="icard">
        ...
        <a class="btn btn-gold download-btn" style="margin-top:8px" href="__DOWNLOAD__">⬇ Télécharger le .exe</a>
      ...
      <div class="icard">
        ...
        <div style="display:flex;gap:10px;margin-top:8px;flex-wrap:wrap">
          <a class="btn btn-purple download-btn" id="mac-arm-btn" href="__RELEASES__" target="_blank" rel="noopener"><i class="bi-download"></i> Apple Silicon</a>
          <a class="btn btn-purple download-btn" id="mac-intel-btn" href="__RELEASES__" target="_blank" rel="noopener"><i class="bi-download"></i> Intel (x64)</a>
        </div>
...
// Modal download redirect
const DL_MODAL = document.getElementById('dl-modal');
const DL_JOIN = document.getElementById('dl-join');
const DL_CONTINUE = document.getElementById('dl-continue');
const DL_SKIP = document.getElementById('dl-skip');
let pendingUrl = '', pendingTarget = '';

document.querySelectorAll('.download-btn').forEach(btn => {
  btn.addEventListener('click', e => {
    e.preventDefault();
    pendingUrl = btn.getAttribute('href') || btn.dataset.url;
    pendingTarget = btn.getAttribute('target') || '_self';
    DL_MODAL.classList.add('show');
  });
});

function goToPending() {
  if (!pendingUrl) return;
  if (pendingTarget === '_blank') window.open(pendingUrl, '_blank', 'noopener');
  else window.location.href = pendingUrl;
  DL_MODAL.classList.remove('show');
}

DL_JOIN.addEventListener('click', () => {
  window.open('https://discord.gg/thQ9k7Qgh7', '_blank', 'noopener');
});

DL_CONTINUE.addEventListener('click', goToPending);
DL_SKIP.addEventListener('click', goToPending);

DL_MODAL.addEventListener('click', e => {
  if (e.target === DL_MODAL) DL_MODAL.classList.remove('show');
});
